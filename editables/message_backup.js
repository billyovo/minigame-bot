...
module.exports= {
    eventTomorrow: function(emote,name,avatar, skyblockTime = '21:00', survivalTime = '22:00'){
        let date = DateTime.now().plus({days: 1}).set({hour: 21});
        let embed = new Discord.MessageEmbed()
        .setColor('#ee831b')
		.setTitle('活動提示')
 //       .setURL('https://billyovo.github.io/event-calendar')
        .addFields(
        { name: '\u200B', value: `${emote} 小遊戲 **${name}** 將於 **明天(${date.toFormat('LL月dd日')})** 進行 ${emote}`},
      //  { name: '\u200B', value: `${emote} 小遊戲 **${name}** 將於 **明天(<t:${date.toSeconds()}:R>)** 進行 ${emote}`},
        { name: '\u200B', value: '__時間__:'},
		{ name: `<:cobblestone:833225746020696075> 空島`, value: skyblockTime, inline: true },
		{ name: `<:grassblock:833226098020057088> 生存`, value: survivalTime, inline: true },
        )
...
:warning: 小遊戲會在空島小遊戲伺服器舉行, 建議提早3分鐘起行以免錯過開始時間
